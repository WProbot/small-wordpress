...
//сортировка постов 
$wp_query = new WP_Query( apply_filters( 'autozone_autos_arg_content_list', $arr ) );
// foreach ($wp_query->posts as  $post) {
//     var_dump($post->ID . ' ' . get_post_meta( $post->ID, '_auto_price', true ));
// }
$url = $_SERVER['REQUEST_URI'];
if ( strstr($url, 'order=_auto_price-desc') || strstr($url, 'order=_auto_price-asc') ) {
    usort($wp_query->posts,'pix_sort_auto_fun');
}
wp_reset_query();
...
// add reg wpquery(начинаются с)
$args['meta_key'] = 'apartment-number';
$args['meta_value'] = '^'.stripslashes($input['apartment_number']);
...
if ($the_query->max_num_pages > 1) {
	for ($i = 2; $i <= $the_query->max_num_pages; $i++) {
	$args = array(
	    'post_type'      => array('product'),
	    'posts_per_page' => 100,
	    'paged'          => $i,
...
$user_posts = $the_query->posts;

for ($paged = 1; $paged < $the_query->max_num_pages + 1; $paged++) {
	$args['paged'] = $paged;
	$the_query = new WP_Query($args);
	$the_query->have_posts();
	$user_posts = $the_query->posts;
	foreach ($user_posts as $key => $u_post) {
        if(!array_key_exists($u_post->ID, $marafon_all_rating)){
...
// запрос постов рейтинга
function marafon_query_rating($value='')
{
   return array(
    'posts_per_page'   => 1000,
    'post_type'   => 'post',
    ...
    'meta_key' => 'marafon_weight_rating',
    'meta_compare' => '>',
